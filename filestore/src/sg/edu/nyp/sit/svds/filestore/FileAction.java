...
public class FileAction {
	...
	public InputStream getFile(String sliceName, int blkSize, byte[] checksum){
		try{
			...
			if(checksum!=null){
				...
				System.arraycopy(SliceDigest.combineBlkHashes(fChk, 0, blkSize, -1), 
						0, checksum, 0, Resources.HASH_BIN_LEN);
			}
	...
	public InputStream getFile(String sliceName, long offset, int len, int blkSize, byte[] checksum){
		try{
			File f=new File(fileRootPath+"/"+sliceName);
			if(!f.exists())
				return null;
			...
			if(len==0){
				...
				/*
				String debug="Read from slice at offset " + offset + ": ";
				for(int i=0; i<readSize; i++){
					debug+=((int)data[i] & 0xff) + " ";
				}
				System.out.println(debug);
				*/
				
				if(readSize==-1)
					in=new ByteArrayInputStream(new byte[0]); //creates a empty stream
				else{
					in=new ByteArrayInputStream(data, 0, readSize);
				}
			}
			
			//System.out.println("[FS] Slice " + sliceName + " 1st byte from offset " + offset + ": " + data[0]);
			//System.out.println("[FS] Slice " + sliceName + " last byte at pos " + (offset + readSize -1) + ": " + data[readSize-1]);
			
			if(checksum!=null){
				System.arraycopy(SliceDigest.combineBlkHashes(
						new File(fileRootPath+"/"+sliceName+".chk"), offset, blkSize, len), 
						0, checksum, 0, Resources.HASH_BIN_LEN);
			}
	...
	public void storeFile(String sliceName, InputStream in, String checksum, String key, int blkSize) throws Exception{
		File f=new File(fileRootPath+"/"+sliceName);
		if(!f.exists())
			f.createNewFile();
	...
	public void storeFile(String sliceName, InputStream in, long offset, String checksum, String key, int blkSize) throws Exception{
		File f=new File(fileRootPath+"/"+sliceName);
		if(!f.exists())
			f.createNewFile();
	...
	private File verifyChecksum(InputStream in, String checksum, String key, int blkSize, long offset, File fChk) throws Exception{
		SliceDigest md=new SliceDigest(new SliceDigestInfo(blkSize, key));
		md.setOffset(offset, null);
		
		//creates a temp file to store the contents as going thru the input stream
		...
		//System.out.println("[FS] Slice (" + fChk.getName()+ ") offset=" + offset+", total="
	...
	private void storeFile (InputStream in, long offset, File f) throws Exception{
	...
	public void storeFileHashes(String sliceName, InputStream in) throws Exception{
		RandomAccessFile f=new RandomAccessFile(fileRootPath+"/"+sliceName+".chk", "rwd");
