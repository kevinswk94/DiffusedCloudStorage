...
public class NamespaceRestlet extends Restlet
{
	...
	private boolean checkClientCert(org.restlet.Request request)
	{
		...
		if (clientAuthentication)
		{
			...
			List<X509Certificate> certs = (List) request.getAttributes().get("org.restlet.https.clientCertificates");
	...
	private void updateFileSliceServer(org.restlet.Request request, org.restlet.Response response)
	{
		...
		FileIOMode mode;
		if (serverId == null || ipAddr == null || strType == null || strMode == null || serverId.length() == 0 || ipAddr.length() == 0 || strType.length() == 0 || strMode.length() == 0)
		{
			response.setStatus(Status.CLIENT_ERROR_BAD_REQUEST, "Missing/Empty arguments.");
			return;
		}
	...
	private void getFileSliceServer(org.restlet.Request request, org.restlet.Response response)
	{
		...
		try
		{
			...
			{
				if (fssi.getKeyId() != null)
					resp.append(FileSliceServerInfo.PropName.KEYID.value() + "=" + Resources.encodeKeyValue(fssi.getKeyId()) + "\n");
				if (fssi.getKey() != null)
					resp.append(FileSliceServerInfo.PropName.KEY.value() + "=" + Resources.encodeKeyValue(fssi.getKey()) + "\n");
			}

			if (fssi.hasProperties())
			{
				for (@SuppressWarnings("rawtypes")
				Map.Entry k : fssi.getAllProperties().entrySet())
				{
					resp.append(FileSliceServerInfo.PropName.OPT.value() + "." + k.getKey().toString() + "=" + Resources.encodeKeyValue(k.getValue().toString()) + "\n");
				}
			}
	...
	private void removeNamespaces(org.restlet.Request request, org.restlet.Response response)
	{
		...
		if (namespaceStr == null || serverId == null || namespaceStr.length() == 0 || serverId.length() == 0)
		{
			response.setStatus(Status.CLIENT_ERROR_BAD_REQUEST, "Missing/Empty arguments.");
			return;
		}
	...
	// Purpose: To inform the master table that it is still alive; and therefore
	// able to
	// service request.
	...
	// Sender: application
	// Purpose: To get the list of available file slice servers so as to send
	// request
	...
	// Sender: file slice servers
	// Purpose: To inform the master table that it is started up and will be
	// able to service
	// request.
	private void registerNamespaces(org.restlet.Request request, org.restlet.Response response)
	{
		...
		try
		{
			...
			memStr = urlQuery.getFirstValue(RestletMasterQueryPropName.Namespace.MEM.value(), true, null); // optional
																											// property
		} catch (Exception ex)
		{
			response.setStatus(Status.CLIENT_ERROR_BAD_REQUEST, "Invalid arguments.");
		...
		FileSliceServerInfo.Type type;
		FileIOMode mode;
		if (namespaceStr == null || serverId == null || ipAddr == null || strType == null || strMode == null || namespaceStr.length() == 0 || serverId.length() == 0 || ipAddr.length() == 0 || strType.length() == 0 || strMode.length() == 0)
		{
			response.setStatus(Status.CLIENT_ERROR_BAD_REQUEST, "Missing/Empty arguments.");
			return;
		}
		...
		try
		{
			...
			String key = na.registerNamespaces((String[]) namespaces.toArray(new String[namespaces.size()]), (avaMem == null ? null : avaMem), serverId, ipAddr, type, true, verOpt, mode, data);
