...
public class Util
{
	...
	{
		return (ba[0] << 24) + ((ba[1] & MASK) << 16) + ((ba[2] & MASK) << 8) + (ba[3] & MASK);
	}
	...
	/**
	 * Generate a value that is unique from an array of integers
	 * 
	 * @param a Matrix of integers to be checked against
	 * @return
	 */
	...
	{
		/*
		 * Conditions for independence xi + yi <> 0 (we can satisfy this if x
		 * and y are positive and non-zero for i <> j, xi <> xj and yi <> yj
		 * (ie, unique)
		 */
	...
	/***
	 * Invert a matrix via Gaussian Jordan Elimination method. Operation using
	 * GF2
	 * 
	 * @param mat
	 * @return
	 */
	public static int[][] invertMatrixGf2(int[][] matIn) throws IDAException
	{
		...
		int[][] mat = new int[size + 1][size * 2 + 1]; // +1 because index 0 is
														// not used
		for (int i = 1; i < size + 1; i++)
		{
			...
			{
				mat[i][j] = matIn[i - 1][j - 1]; // -1 because matIn is 0 index
													// based
			}
		...
		for (int r = 0; r < n; r++)
		{
			for (int c = 0; c < n; c++)
			{
				result[r][c] = mat[r + 1][c + n + 1]; // for some reason 0 index
														// is not used.
			}
		}
	...
	/**
	 * divide row i by A[i][j] pre: A[i][j]!=0, A[i][q]==0 for 1<=q<j post:
	 * A[i][j]==1;
	 * 
	 * @param A
	 * @param i
	...
	/**
	 * subtract an appropriate multiple of row i from every other row pre:
	 * A[i][j]==1, A[i][q]==0 for 1<=q<j post: A[p][j]==0 for p!=i
	 * 
	 * @param A
	 * @param i
	 * @param j
	 ...
	 * @throws IDAException
	 */
	private static int[][] getCofactorElement(int[][] values, int row, int col) throws IDAException
	{
