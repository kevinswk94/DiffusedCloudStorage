...
public class RabinImpl2 implements IInfoDispersal
{
			...
			throws IDAException
	{
		...
		for (int n = 0; n < sliceLen; n++)
		{
			...
			{
				keyrow = info.getMatrix()[row]; // get the correct row in the
												// key matrix
	...
	public List<InputStream> split(InputStream is, IdaInfo info) throws IDAException
	{
		...
		// Tag the segment number and size of stream at the front, they are
		// needed when combining the file
	...
	public InputStream combine(List<InputStream> slices, IdaInfo info) throws IDAException
	{
		...
		// IdaInfo class helps in managing parameters for the IDA algorithm
		// (size, lengths, mod value etc)
		// IdaInfo idainfo = new IdaInfo(5,3);
		int m = info.getQuorum(); // if the input number of stream < m,
									// algorithm cannot function
		...
		// Retrieve the id of each slice. This is required to get the correct
		// row in the key matrix
		int[] sliceIds = null;
		int[][] inverse = null;
		int[] bytesFromSlices = null;
		// use to track the number of bytes of output, this will remove adding
		// padding bytes added previously
		// int nOutputCount = 0;
		...
		for (int i = 0; i < info.getSliceLength(); i++)
		{
			...
			for (int j = 0; j < m; j++)
			{
				...
				{
					// System.out.println("Output Count = " + nOutputCount +
					// ", StreamSize = " + nStreamSize);
					break;
				}
			}
		}
	 ...
	 * 
	 * @return Array of bytes of the file. Note that it is in int form because
	 *         in Java, bytes are SIGNED! That is, from -127 to 128. As such we
	 *         cannot calculate it based on byte and must first convert it to
	 *         integer
	 * @throws IOException
	 */
...
//	}
	 ...
	 * tagSegmentExtraData() function during the combine operation
	 * 
	 * @param lst
	 * @param SliceId
	...
	private int[] parseSegmentExtraData(List<InputStream> lst) throws IDAException
	{
		...
		try
		{
			...
			// throw new
			// IDAException("parseSegmentExtraData::Stream Size check failed.");
			// }
	...
	/***
	 * Generates the inverse matrix used to recombine the slices. The rows
	 * needed depends on the slices used to perform the recombination.
	 * 
	 * @param sliceIds
	 *            An array of the IDs of the slices used for the recombination.
	 * @param key
	 *            The key matrix used during the splitting.
	...
	private int[][] generateInverse(int[] sliceIds, int[][] key) throws IDAException
	{
		...
		// Extract the rows of the key matrix corresponding to the input
		// sliceIds parameter
		int index = 0;
		for (int i = 0; i < sliceIds.length; i++)
		{
	...
	public void split(ArrayBlockingQueue<Integer> in, BlockingQueue<Integer>[] out, IdaInfo info)
	{
		(new Splitter(in, out, info)).start();
	}

	public void combine(BlockingQueue<int[]>[] in, ArrayBlockingQueue<Integer> out, IdaInfo info)
	{
	...
	private class Splitter extends Thread
	{
		...
		public Splitter(ArrayBlockingQueue<Integer> in, BlockingQueue<Integer>[] out, IdaInfo info)
		{
			this.in = in;
			this.out = out;
			this.info = info;
		...
		public void run()
		{
			...
			// if the output stream is not closed or the buffer still have data
			// waiting to be read,
			// read m number of bytes from the buffer, transform it and put in
			// the out queue for
			...
			while (!done)
			{
				...
				for (byteCnt = 0; byteCnt < info.getQuorum(); byteCnt++)
				{
					...
					try
					{
						...
						// if the buffer receive a -1, means the client has
						// finish writing
						// so break out of the for loop. The byteCnt will store
						// how many bytes
			...
			// put -1 in the out queues to indicate that transformation has
			// finished
	...
	private class Combiner extends Thread
	{
		...
		public Combiner(BlockingQueue<int[]>[] in, ArrayBlockingQueue<Integer> out, IdaInfo info)
		{
			this.in = in;
			this.out = out;
			this.info = info;
		...
		public void run()
		{
			...
			while (!done)
			{
				...
				// read m array of bytes from the in queues together with the
				// seq
				...
				// file slices are not read 1 byte at a time but multiple of
				// bytes
				// (therefore result in array of array of bytes) to reduce no of
				// http
				// calls. So in order to transform, place in single byte array
				// first
				for (int index = 1; index < buff[0].length; index++)
				{ // NOTE: starts from 1 because 1st byte is used to indicate
					// seq
					int[] tmp = new int[info.getQuorum()];
					for (int i = 0; i < info.getQuorum(); i++)
					{
						// -1 in the slice to indicate end of slice has been
						// reached,
					...
					try
					{
						...
						/*
						 * System.out.println("Display inverse matrix"); for(int
						 * i=0; i<inverse.length; i++){ for(int n=0;
						 * n<inverse[i].length; n++)
						 * System.out.print(inverse[i][n] + " ");
						 * 
						 * System.out.println(); }
						 * 
						 * String str="To be combined: "; for(int i=0;
						 * i<tmp.length; i++){ str+= tmp[i] + " "; }
						 * System.out.println(str);
						 */

						for (count = 0; count < info.getQuorum(); count++)
						{
							// perform the combination here and put the result
							// in var data
							data[count] = mac(tmp, inverse[count]);
							nOutputCount++;
							// System.out.println("count: " + count+
							// ", output cnt: " + nOutputCount + ", data size: "
							// + info.getDataSize());
					...
					} catch (IDAException e)
					{
						...
						// set to true so will put the error code in the out
						// queue
						error = true;
						break;
					}
					...
					{ // based on count to handle case of padding removal
						try
						{
							// debug+=data[i]+ " ";
			...
			try
			{
				out.put((!error ? FileInfo.EOF : IInfoDispersal.ERROR_CODE));
			} catch (InterruptedException e)
			{
			}
		}
